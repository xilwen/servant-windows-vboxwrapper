...
bool VBoxAppliance::exporting(false);
...
void VBoxAppliance::exportOVA(std::wstring uuid, std::wstring path)
{
	if (busy)
	{
		throw std::runtime_error("VBoxAppliance is busy, try again later");
	...
	succeeded = false;
	exporting = true;	
	auto fullPath(L'\"' + vBoxManagePath + L'\"' + L" export " + uuid + L" -o " + path + L" 2>&1");
	if ((consoleOutputPipe = _wpopen(fullPath.c_str(), L"rt, ccs=UNICODE")) == nullptr)
	{
		throw std::runtime_error("Can not start VBoxManage process");
	}
...
void VBoxAppliance::childOutputHandler()
{
	...
	if (exporting)
	{
		outputString += L"100%."; //export only have 1 pass
	}
	progress = 0;
	auto bufferLimit(20);
	...
	if (feof(consoleOutputPipe))
	{
		...
		Logger::log(className, __func__, InfoLevel::INFO, outputString);
